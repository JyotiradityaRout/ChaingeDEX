...
import FRC758 from '../../build/FRC758.json'
...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  // deploy tokens
  const tokenA = await deployContract(wallet, FRC758, [expandTo18Decimals(10000)])
  const tokenB = await deployContract(wallet, FRC758, [expandTo18Decimals(10000)])
  const WETH = await deployContract(wallet, WETH9)
  const WETHPartner = await deployContract(wallet, FRC758, [expandTo18Decimals(10000)])
